<!---<cfcache timespan="#CreateTimespan(1, 0, 0, 0)#" directory="page_cache">
...
<cfquery name="get_all_employee_PR_info" datasource="jrgm">
 SELECT   ds_id, supervisor, in_out_status, time_out FROM APP_Employee_Payroll_Clock   WHERE ID  > 270000
  </cfquery>
...
<cfquery name="get_daily_sheets" datasource="jrgm" maxrows="200"> 
 SELECT  ds_approved ,CREW_LEADER_ID,DS_DATE, ID, SUPERVISOR_ID  FROM app_daily_sheets 
WHERE  <!---(Supervisor_ID IN (#myList#)  OR  Supervisor_ID = #SESSION.userid#)---> branch_code =#branchcode#
...
SELECT      ID, Employee_ID,  employee_name, access_role, employee_active, firstname, lastname, employee_branch
FROM         app_employee_passwords WHERE employee_branch = '#SESSION.branch#'
