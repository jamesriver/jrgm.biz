 ...
 <cfif NOT IsDefined("url.approved")>
 <cfquery name="get_employees_with_time_approved" datasource="jrgm"  >
		...
		WHERE branch = '#SESSION.branch#'  AND (app_employee_payroll_clock.Time_In > '#DateFormat(pay_period_start, "yyyy-mm-dd")# 00:00:00.000' 00:00:00.000' AND app_employee_payroll_clock.Time_Out < #pay_period_end#) AND (payroll_approved = 0 OR payroll_approved IS NULL)
		ORDER by APP_employees.last_name
 	</cfquery>
    
    <cfif get_employees_with_time_approved.recordcount EQ 0>
    All Payroll is Approved<br>
   <!---   <a href="payroll_approved.cfm?approved=yes">See Payroll</a> --->
    <cfabort>
    <cfelse>
    Payroll is not approved. <a href="payroll_view_employee_detail_1week.cfm?pay_period_number=1">Go here to approve payroll</a>
      <cfabort>
    </cfif>
  </cfif>
    ...
    <div class="centrecontent_inner">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="sortable" >
        ...
        <!--- <CFSET begin_date = '04/27/2014'>
         <CFSET midplus1_date = '05/04/2014'>
		...
		INNER JOIN APP_employees
		ON app_employee_payroll_clock.Employee_ID=APP_employees.[Employee ID]  
		WHERE branch = '#SESSION.branch#'  AND app_employee_payroll_clock.Time_In > '#DateFormat(pay_period_start, "yyyy-mm-dd")# 00:00:00.000' 00:00:00.000'
		ORDER by APP_employees.last_name
 </cfquery>
		  ...
		  <cfset hourssum = int(sumtime\60)>
           ...
           <td align="center" class="yellowaltrt">#hourssum#:#NumberFormat(minutessum,"09")#</td></cfoutput>
             ...
             <cfquery name="get_employee_time_sum_w2" datasource="jrgm" >
				SELECT Employee_ID,SUM(time_worked) AS sumtime FROM 
			app_employee_payroll_clock WHERE Employee_ID =#get_employees_with_time.Employee_ID#  AND app_employee_payroll_clock.Time_In > #midplus1_date#   AND app_employee_payroll_clock.Time_Out <  				#end_date#
AND in_out_status =2
