...
WHERE    [Employee ID] = #form.crew_assignment_last#
   ...
   <cfif  IsDefined("CREW_ASSIGNMENT_LAST") >
 <cfquery name="get_filter_employee" datasource="jrgm">
SELECT   [Employee ID],  [Name FirstLast]  AS employee_name FROm app_employees
WHERE  [Employee ID] = #form.crew_assignment_last#
 </cfquery>
...
<cfif IsDefined("form.inventory_status") AND form.inventory_status LT 4 > AND inventory_status ='#form.inventory_status#'</cfif>
 ...
 ORDER by  ID ASC
   ...
   <cfif   IsDefined("Form.Pend")  >
 <div class="inventory-message">Your Inventory Report is Pended.<br /><br />
<a href="default.cfm" class="bluebutton">Go to Home Screen</a>  <a href="inventory_list_printable_eversion1.cfm?Inventory_Report_ID=<cfoutput>#Inventory_Report_ID#</cfoutput>" class="bluebutton">Continue With the Report</a></div>
 <cfabort>
 </cfif>
...
User:  <cfoutput>#SESSION.screenname#</cfoutput>
  <cfif  IsDefined("form.filter")  OR IsDefined("CREW_ASSIGNMENT_LAST")> <br />
...
<div class="inventory-third-last">
   <cfif NOT IsDefined("form.filter") AND     NOT IsDefined("CREW_ASSIGNMENT_LAST")>
         <form action="inventory_list_printable_eversion1.cfm" method="post"> 
        Filter:
