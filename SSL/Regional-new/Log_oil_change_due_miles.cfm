  ...
  <div class="page-head">
    <div class="container-fluid"> 
      ...
      <div class="page-title">
        <h1><cfoutput>#regionname#</cfoutput> Region Oil Change Due - Miles</h1>
      </div>
      <!-- END PAGE TITLE --> 
      <!-- BEGIN PAGE TOOLBAR -->
      <div class="page-toolbar">
        <ul class="page-breadcrumb breadcrumb">
          ...
          <li class="active"> <cfoutput>#regionname#</cfoutput> Region Oil Change Due - Miles</li>
        </ul>
      </div>
      <!-- END PAGE TOOLBAR --> 
    </div>
...
SELECT     ID,   Category, Product_description, Product_name,   Equipment_ID, 
   hours_mileage, mileage_eq, hours_eq, oil_change_date_due, 
                       oil_change_mileage_due, last_oil_change_date,  last_oil_change_miles, last_oil_change_employee_id, 
   oil_change_criteria , Branch_code,BRANCH_NAME,CREW_ASSIGNMENT_LAST
FROM         equipment where hours_mileage = 2  AND inventory_status > 0 <!---AND   oil_change_date_due IS NOT NULL ---> AND Branch_name IN   (#PreserveSingleQuotes(branchlist)#)
ORDER BY ID ASC
</cfquery>
              <!-- BEGIN EXAMPLE TABLE PORTLET-->
...
WHERE  employee_ID =  #Crew_assignment_last# 
         ...
         <td>#Category#</td>
     <td><a href="history_allocation.cfm?ID=#ID#">#equipment_id#</a></td>
     <cfif last_oil_change_date IS "">
     <td>   <span class="red">None</span></td>
          <cfelse>
     <td>#dateformat(last_oil_change_date,"mm/dd/yy")#</td>
     </cfif>
     <td>#NumberFormat(last_oil_change_miles,"999,999")#</td>
              <td>#NumberFormat(oil_change_mileage_due,"999,999")#</td>
              <td>#NumberFormat(mileage_eq,"999,999")#</td>
              <td><cfif  mileage_eq GT oil_change_mileage_due>
