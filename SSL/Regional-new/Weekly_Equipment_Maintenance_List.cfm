...
SELECT      Inspection_ID, Crew_LeaderID, Inspection_Date, Inspection_Type, Problem, Created_By, Created_On, Current_Mileage, vehicleSatisfactory, 
 InspectionMode
FROM     app_Inspection_Master  WHERE Inspection_Type ='Weekly'   AND <!--- Crew_LeaderID IN  (#myList2#)--->branch_code IN (#branchcodelist#)
order by  Inspection_Date DESC, Inspection_ID DESC
</cfquery>
      <div class="page-title">
        <h1>Weekly Equipment Maintenance List</h1>
      </div>
      <!-- END PAGE TITLE --> 
      <!-- BEGIN PAGE TOOLBAR -->
