<cflocation url="dailysheet_add2.cfm">
...
<CFIF IsDefined("url.work_date")>
 ...
 <cfset tomorrow = dateadd("d",1,somedate)>
  ...
  <cfif IsDefined("form.CREW_LEADER_ID") AND  IsDefined("form.submit") AND  IsDefined("form.ds_date") AND  form.ds_date NEQ "" AND form.ds_date LT  todayDate AND  IsDefined("form.SUPERVISOR_ID")>
  <cfquery name="insert_ds" datasource="#request.dsn#">
 INSERT INTO  app_daily_sheets (Supervisor_ID, Crew_Leader_ID, DS_Date, record_created,entry_method,branch_code) VALUES ( #form.Supervisor_ID#, '#form.CREW_LEADER_ID#','#form.ds_date#',CURRENT_TIMESTAMP,2,#form.branchcode#  )
 </cfquery>
...
WHERE  branch ='#form.branch_name#'  AND    active_record = 1  
   ...
   <div class="page-content">
		<div class="container-fluid">
			...
			<!-- BEGIN PAGE CONTENT INNER -->
 					<div class="row">
					  <div class="col-md-6">
       <cfif IsDefined("form.CREW_LEADER_ID") AND  IsDefined("form.submit") AND  IsDefined("form.ds_date") AND  form.ds_date NEQ "" AND form.ds_date GT  yesterday AND  IsDefined("form.SUPERVISOR_ID")>
 <br />
 <h2>The Daily Sheet date must be before today. <br />
 Please <a href = "javascript:history.back()">go back</a> and select a date before today</h2>
 <cfabort>
 <br />
 </cfif>
       ...
       <br />
 <h2>Daily Sheet ID : <cfoutput>#getmaxid.newid# has been created.</cfoutput></span><cfoutput><br /><br />
 
<a href="daily_sheet_edit.cfm?dsid=#getmaxid.newid#&message=yes" class="btn btn-primary" >Edit Daily Sheet</a> </cfoutput></h2><br />
        <br />
...
<cfelse>
       ...
       <form action="dailysheet_add.cfm" method="post">
          <table class="table large">
        <tr>
              <td  align="right">Choose Branch :</td>
              <td>
                  <select name="BRANCH_NAME"   onchange='this.form.submit()'  class="bs-select form-control">
                    <option value="Choose"   
       ...
       <form action="dailysheet_add.cfm" method="post">
          <table class="table large">
             <tr>
              ...
              <td>  <input class="form-control form-control-inline input-medium date-picker" type="text" name="ds_date" />
              </td>
            </tr>
            <tr>
              <td nowrap="nowrap" align="right">Choose Supervisor : </td>
            ...
            <tr>
              <td nowrap="nowrap" align="right">Choose Crew Leader : </td>
...
<input type="submit" name="submit"   value="SUBMIT" class="btn btn-primary" />
 ...
 <cfif FORM.BRANCH_NAME EQ 'Richmond' >
 <CFSET branchcode =10>
     <cfelseif FORM.BRANCH_NAME EQ 'Portsmouth' >
    <CFSET branchcode =20>
      <cfelseif FORM.BRANCH_NAME EQ 'Charlottesville' >
    <CFSET branchcode =30>
      <cfelseif FORM.BRANCH_NAME EQ 'Williamsburg' >
    <CFSET branchcode =70>
      <cfelseif FORM.BRANCH_NAME EQ 'Chesterfield' >
    <CFSET branchcode =80>
      <cfelseif FORM.BRANCH_NAME EQ 'Newport News' >
    <CFSET branchcode =90>
     <cfelse>
   <CFSET branchcode =0>
  </cfif>
 <input type="hidden" name="branchcode"   value="<cfoutput>#branchcode#</cfoutput>"  />
 </td>
            </tr>
            <tr>
...
<!-- BEGIN CORE PLUGINS --><div class="scroll-to-top"> <i class="icon-arrow-up"></i> </div>
<!-- END FOOTER -->
<!-- BEGIN JAVASCRIPTS(Load javascripts at bottom, this will reduce page load time) -->
<!-- BEGIN CORE PLUGINS -->
<!--[if lt IE 9]>
