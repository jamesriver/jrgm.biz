<cfif IsDefined("form.EMPLOYEEID") >
 ...
 INSERT INTO app_employee_payroll_clock (Employee_ID,crew_leader,supervisor,ds_id, In_Out_Status, time_worked,ds_date,entry_method,record_created) 
 VALUES (#form.EMPLOYEEID#,#form.crew_leader#,#form.supervisor#,#dsid#,2,0,'#form.ds_date#',2,CURRENT_TIMESTAMP)
 </cfquery>
...
WHERE branch = '#url.branch#'  AND    active_record = 1 AND  [Employee ID] =#form.EMPLOYEEID#
</cfquery>
...
SELECT first_name,last_name,branch,position,[Employee ID] As employee_id,  [Name FirstLast] AS fullname  FROM APP_employees
WHERE branch = '#url.branch#'  AND    active_record = 1 AND  [Employee ID] NOT IN (#mylist#)
        ...
        <table border="0" cellspacing="0" cellpadding="0" width="90%" >
          <tr>
            <td valign="top"><table border="0" cellspacing="0" cellpadding="0" >
                <tr>
                  <td nowrap="nowrap" class="statusTD"><strong>Choose employee to add:</strong></td>
                  <td class="statusTD_right"><div class="stylededit-select"><select name="employeeID" >
              ...
              <div align="right"> <cfoutput>
                ...
                <input type="submit" name="submit" id="submit" value="Submit"  class="materialsbutton"/>
                <button type="button" onclick="javascript:window.close()"  class="materialsbutton">Close</button>
              </div>
              </p></td>
          </tr>
        </table>
