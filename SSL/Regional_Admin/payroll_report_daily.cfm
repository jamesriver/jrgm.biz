...
<cfquery name="calculate_time" datasource="jrgm">
    ...
    WHERE in_out_status = 1 AND SUPERVISOR  IN (#mylist#)
    </cfquery>
...
<link href="../Supervisors/css/styles.css" rel="stylesheet" type="text/css" />
<link href="../Supervisors/css/bootstrap.css" rel="stylesheet">
<!--[if lt IE 9]>
...
SELECT  DISTINCT ds_date FROM app_daily_sheets   WHERE SUPERVISOR_ID  IN (#mylist#)
...
<br />
<div class="header">Todays Employee List for <cfoutput>#regionname#</cfoutput> Region</div>

  <cfif IsDefined("form.DATEREQUESTED")>
  <CFSET DateRequested = (#form.DATEREQUESTED#)> 
  </cfif>
  <cfquery name="get_daily_sheets" datasource="jrgm"   >
SELECT  DISTINCT employee_ID FROM app_employee_payroll_clock   WHERE ds_date = '#form.DateRequested#' AND SUPERVISOR   IN (#mylist#)
 </cfquery>
    <cfquery name="get_number_daily_sheets" datasource="jrgm"   >
SELECT  DISTINCT ID FROM app_daily_sheets   WHERE ds_date = '#DateRequested#' AND SUPERVISOR_ID   IN (#mylist#)
 </cfquery>
  <strong><cfoutput>  <span class="arialfont">#get_daily_sheets.recordcount# </span></cfoutput><span class="arialfont"> employees working on <cfoutput>  #get_number_daily_sheets.recordcount#</cfoutput> crews on <cfoutput>  #DateFormat(form.DATEREQUESTED, "mm/dd/yyyy")#  as of  #TimeFormat(Now_Time, "hh:mm tt")#</cfoutput></span></strong><br /> 
     <br />
...
(SELECT Employee_ID FROM  app_employee_payroll_clock WHERE  ds_date ='#form.DATEREQUESTED#' AND SUPERVISOR   IN (#mylist#))
...
WHERE Employee_ID =#get_employees_for_today.Employee_ID# 
 AND ds_date ='#form.DATEREQUESTED#' AND SUPERVISOR IN (#mylist#)
ORDER by ID ASC
                </cfquery>
                <cfif get_employee_dsid.recordcount EQ 1>
                 <td align="center"  class="dstable"><a href="../Supervisors/daily_sheet.cfm?dsid=#get_employee_dsid.ds_id#">#get_employee_dsid.ds_id#</a></td>
                <cfelse>
...
WHERE Employee_ID =#get_employees_for_today.Employee_ID# 
AND ID  <> #get_employee_time_am.ID#  AND ds_date ='#form.DATEREQUESTED#' AND ds_id =#get_employee_dsid.ds_id# AND SUPERVISOR IN (#mylist#)
ORDER by ID DESC
                </cfquery>
...
SELECT Sum(time_worked) AS  minutes_worked_day FROM  app_employee_payroll_clock WHERE Employee_ID =#get_employees_for_today.Employee_ID#  
 AND ds_date ='#form.DATEREQUESTED#' AND ds_id =#get_employee_dsid.ds_id# AND SUPERVISOR IN (#mylist#)
                </cfquery>
                <!---  <cfdump var="#get_employee_minutes_for_day#"> --->
                <cfquery name="get_employee_minutes_for_day_inprogress" datasource="jrgm" maxrows="1">
SELECT Sum(time_worked_current) AS  minutes_worked_day_inprogress FROM  app_employee_payroll_clock WHERE Employee_ID =#get_employees_for_today.Employee_ID# AND in_out_status =1    AND ds_date ='#form.DATEREQUESTED#' AND ds_id =#get_employee_dsid.ds_id# AND SUPERVISOR IN (#mylist#)
                </cfquery>
...
<script src="../Supervisors/js/twitter-bootstrap-hover-dropdown.js"></script>
</body>
</html>
