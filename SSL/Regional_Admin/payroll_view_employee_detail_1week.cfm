...
<cfset tomorrow = dateadd("d",1,somedate)>
 ...
 <cfquery name="get_employees_with_time" datasource="jrgm"  >
		...
		WHERE branch = '#url.branch#'  AND (app_employee_payroll_clock.Time_In > '#DateFormat(pay_period_start, "yyyy-mm-dd")# 00:00:00.000' AND app_employee_payroll_clock.Time_Out < '#DateFormat(pay_period_end_week_plusone, 'yyyy-mm-dd')#')
...
<cfparam name="employee_total" default="0">
 ...
 UPDATE App_Employee_Payroll_Clock SET payroll_approved = NULL, approved_by = NULL WHERE   Employee_ID IN (#employeelist#)  AND (app_employee_payroll_clock.Time_In > '#DateFormat(pay_period_start, "yyyy-mm-dd")# 00:00:00.000' AND app_employee_payroll_clock.Time_Out < '#DateFormat(pay_period_end_week_plusone, 'yyyy-mm-dd')#')
 </cfquery>
 <CFSET mylist = #Form.EMPLOYEE_ID#>
<cfloop  list = "#mylist#"   index = "i" >
 
  <cfquery name="update_time" datasource="jrgm">
 UPDATE App_Employee_Payroll_Clock SET payroll_approved = 1, approved_by = #SESSION.userid#  WHERE   Employee_ID = #i# AND (app_employee_payroll_clock.Time_In > '#DateFormat(pay_period_start, "yyyy-mm-dd")# 00:00:00.000' AND app_employee_payroll_clock.Time_Out < '#DateFormat(pay_period_end_week_plusone, 'yyyy-mm-dd')#')
 </cfquery></cfloop>
 <CFSET flag = 'uncheck'>
...
SELECT Employee_ID,  time_worked, in_out_status,ds_date 
 ...
 WHERE Employee_ID IN (#employeelist#) AND app_employee_payroll_clock.Time_In >= #pay_period_start# AND  app_employee_payroll_clock.Time_In < #end_date_plus1#
 AND in_out_status =2
  </cfquery> 
          ...
          <cfloop query="get_employees_with_time" >
             <cfoutput><tr>
                <td nowrap="nowrap"><a href="payroll_view_employee_detail2.cfm?empid=#get_employees_with_time.Employee_ID#&branch=#url.branch#&pay_period_number=<cfoutput>#pay_period_end_week_L#</cfoutput>">#get_employees_with_time.first_name# #get_employees_with_time.last_name# </a></td>
              </cfoutput>
...
SELECT  Employee_ID , SUM(time_worked) As sumdailytime, in_out_status,ds_date 
 ...
 GROUP by Employee_ID, ds_date, in_out_status
              ...
              <td align="center"><cfquery name="is_already_approved" datasource="jrgm">
       SELECT  payroll_approved
       FROM app_employee_payroll_clock
       WHERE "Employee_ID" =#get_employees_with_time.Employee_ID# AND (app_employee_payroll_clock.Time_In > '#DateFormat(pay_period_start, "yyyy-mm-dd")# 00:00:00.000' AND app_employee_payroll_clock.Time_Out < '#DateFormat(pay_period_end_week_plusone, 'yyyy-mm-dd')#')
         </cfquery>
