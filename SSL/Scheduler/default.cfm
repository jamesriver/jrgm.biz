...
SELECT DISTINCT Employee_ID, crew_name AS employee_name,supervisor_id,Employee_Position_ID FROM app_crews WHERE (supervisor_id = #SESSION.userid#  OR Employee_ID = #SESSION.userid#) AND Employee_Position_ID
...
FROM app_crews WHERE 
  ...
  OR (crew_leader_id = #get_employees.employee_id#  
  AND (start_date  >=  (#today#) AND start_date < (#tomorrow#))) 
  OR  (crew_leader_id = #get_employees.employee_id#   AND end_date > (#tomorrow#))
AND active_record =1  
ORDER by Employee_Position_ID DESC 
 ...
 --->
           ...
           <cfloop query="get_employees_cl">
                      <li><a href="employee_details.cfm?employee_id=#get_employees_cl.Employee_ID#&work_date=#todayDate#">#employee_name#</a></li>
                    </cfloop>
                  </ul></td>
