<cfif IsDefined("form.ds_id") AND  IsDefined("form.submit")  AND  form.submit NEQ "Delete"  >
  <cfif  form.QUANTITY_USED  IS "">
    <cfoutput>
      <cflocation url="daily_sheet_edit_materials1.cfm?dsid=#ds_id#&job_clock_id=#form.job_clock_id#&error=qty">
    </cfoutput>
  </cfif>
...
<cfif IsDefined("url.job_clock_id")>
  <CFQUERY name="get_this_job" datasource="#request.dsn#">
  SELECT * FROM app_job_clock WHERE ID = #url.job_clock_id#
   </CFQUERY>
</cfif>
<cfif IsDefined("form.job_clock_id")>
  <CFQUERY name="get_this_job" datasource="#request.dsn#">
  SELECT * FROM app_job_clock WHERE ID = #form.job_clock_id#
   </CFQUERY>
</cfif>
      ...
      <div class="row">
        <div class="col-md-12">
          <cfif IsDefined("form.ds_id") AND  IsDefined("form.submit")  AND  form.submit NEQ "Delete"  >
            ...
            <p><cfoutput> <a href="daily_sheet_edit.cfm?DSID=#form.ds_id#" class="btn btn-primary">Back to Daily Sheet</a></cfoutput></p>
            <cfabort>
          </cfif>
          <cfif IsDefined("form.ds_id") AND  IsDefined("form.submit")  AND  form.submit EQ "Delete"  >
...
SELECT *  FROM app_materials_list_OLD order by sortid
</cfquery>
            <table class="table large">
              <tr>
                <td><strong>Select Materials</strong></td>
                <td><select name="item_id" class="bs-select form-control" >
                    <cfoutput>
                      <option value="#get_this_material.item_id#"  selected>#get_this_material.item_id#</option>
                    </cfoutput> <cfoutput query="get_all_materials">
                      <option value="#item_id#">#item_id#</option>
                    </cfoutput>
                  </select></td>
              </tr>
              <tr>
                <td><strong>Select Unit</strong></td>
                <td><select name="unit_used" class="bs-select form-control">
              ...
              <tr>
                <td >&nbsp;</td>
                <td><input type="submit" name="submit" class="btn btn-primary" id="submit" value="Submit" />
                  <input type="submit" name="submit" class="btn btn-primary" id="submit" value="Delete" /></td>
              </tr>
              <cfoutput>
          ...
          <cfoutput> <a href="daily_sheet_edit.cfm?dsid=#ds_id#" class="btn btn-primary">Back to Daily Sheet</a> </cfoutput> </div>
      </div>
