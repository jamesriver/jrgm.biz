<!--- This code is for the prior day timeout issue --->
 ...
 SELECT * FROM APP_Employee_Payroll_Clock WHERE In_out_status = 1 AND ds_date < '#today_datex#'
   ...
   <!---  <cfdump  var="#time_me_out#"> --->
    ...
    <cfmail to="pat@1design.com"    FROM="JRGM Alerts <alerts@jrgm.com>"  subject="Timed in Prior Supervisor"  type="html">
 Timed in Prior Supervisor  - #time_me_out.ds_id#
    </cfmail> 
 ...
 <!--- END  This code is for the prior day timeout issue --->
...
Crew Leader : <cfoutput>#get_crew_leader.employeename#</cfoutput></td>
          ...
          <cfoutput><a href="daily_sheet_edit_add_job1.cfm?ds_id=#dsid#" class="greenbuttonsm">Add a Job</a></cfoutput>
            </tr>
          </table>
...
WHERE Employee_ID =#get_employees_for_Crew_Leader.Employee_ID# AND ds_id = #dsid# AND in_out_status =1
               ...
               <cfif ds_date GT #APPLICATION.blockdate#>
             <tr>
            <cfoutput>  <td align="left" colspan="8" class="dstable">
             <a href="daily_sheet_edit_add_employee.cfm?" onclick="javascript:void window.open('daily_sheet_edit_add_employee.cfm?dsid=#dsid#','1384819222444','width=800,height=200,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=1,left=300,top=20');return false;"><i class="fa-green fa-plus-square"></i><span class="greentype">Add Employee</span></a>
 </td></cfoutput>
            </tr>
            </cfif>
          </table>
   ...
   It needs to loop through the each employee job_services  --->
		...
		AND ds_id = #dsid# 
              ...
              <tr>
				...
				<cfoutput>
                <a href="daily_sheet_edit_add_materials1.cfm?dsid=#dsid#&job_clock_id=#get_this_job.ID#"><i class="fa-green fa-plus-square"></i><span class="greentype">Add Materials</span></a></cfoutput>
