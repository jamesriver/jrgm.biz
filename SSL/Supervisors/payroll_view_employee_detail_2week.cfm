...
<div id="outerfull">
 ...
 <cfquery name="get_employees_with_time" datasource="jrgm"      >
		...
		WHERE branch = '#SESSION.branch#'  AND app_employee_payroll_clock.Time_In > '#DateFormat(pay_period_start, "yyyy-mm-dd")# 00:00:00.000' AND  app_employee_payroll_clock.Time_In < #end_date_plus1#
...
<cfloop query="get_employees_with_time">
 ...
 AND in_out_status =2
        ...
        <cfloop query="get_employees_with_time" >
          <tr> <cfoutput>
              <td nowrap="nowrap"><a href="payroll_view_employee_detail2.cfm?empid=#get_employees_with_time.Employee_ID#&pay_period_number=#pay_period_number#" target="_blank">#get_employees_with_time.first_name# #get_employees_with_time.last_name# </a></td>
            </cfoutput>
		  ...
		  <cfset hourssum = int(sumtime\60)>
           ...
           <td align="center" class="yellowaltrt">#hourssum#:#NumberFormat(minutessum,"09")#</td></cfoutput>
             ...
             <cfquery name="get_employee_time_sum_w2" datasource="jrgm" >
				SELECT Employee_ID,SUM(time_worked) AS sumtime FROM 
			app_employee_payroll_clock WHERE Employee_ID =#get_employees_with_time.Employee_ID#  AND app_employee_payroll_clock.Time_In > '#DateFormat(midplus1_date, "yyyy-mm-dd")# 00:00:00.000'   AND app_employee_payroll_clock.Time_Out <  								            #pay_period_endPLUS1_DATE#
AND in_out_status =2
