...
<!---cfoutput>
    SELECT ajsae.Job_ID, ajsae.Service_ID, ajsae.Employee_ID, ajsae.Total_Time, ajsae.Service_Time_In, ajsae.ds_id FROM dbo.app_job_services_actual_employee ajsae
    WHERE 1=1
    ...
    <cfif IsDefined('form.start_date') AND IsDefined('form.end_date')>
        <cfif form.start_date NEQ '' AND form.end_date NEQ ''>
            AND ajsae.Service_Time_In>='#form.start_date#'
            AND ajsae.Service_Time_In<'#form.end_date#'
        </cfif>
    </cfif>
    <cfif IsDefined('form.employee')>
        <cfif form.employee NEQ ''>
            AND ajsae.Employee_ID='#form.employee#'
        </cfif>
    </cfif>
