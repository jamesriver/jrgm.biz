...
<cfif IsDefined('form.job_id') AND IsDefined('form.biz_approved_date')>
    <cfquery name="main_query" datasource="jrgm">
        ...
        WHERE qs.[Job ID]=<cfqueryparam value="#form.job_id#" CFSQLType="CF_SQL_INTEGER">
    ...
    <cfloop query="main_query">
        <cfset output_str = output_str & opportunity_id & '|' & Adjusted_Grounds_Maintenance_Contract & '|' & Irrigation_Contract & '|' & Seasonal_Color_Contract & '|' & contract_start_date & '|' & contract_end_date & '|' & contract_installments & '|' & opportunity_id_original & '|' & biz_approved_date & '|' & user_id & '|'>
    </cfloop>
</cfif>
