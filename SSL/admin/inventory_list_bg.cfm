...
SELECT inventory_status ,ID,Product_description,Product_name,Serial_Number,Equipment_ID,category,License_Plate,Equipment_Year, engine_cycle, mileage_eq,hours_eq,BRANCH_NAME,ISNULL(CREW_ASSIGNMENT_LAST,0),Date_Purchased
...
ORDER by  ID ASC
     ...
     <div class="header"> <cfif IsDefined("form.BRANCH_NAME") AND form.BRANCH_NAME NEQ "All" ><cfoutput>#form.BRANCH_NAME#</cfoutput> <cfelse>JRGM</cfif>
    Equipment List  
      <cfif IsDefined("form.category") AND form.category NEQ "All" ><cfoutput>(#form.category#)</cfoutput></cfif>  <span class="record-count"><cfoutput>#getequipment.recordcount#</cfoutput> Records&nbsp; &nbsp;
             ...
             <cfif session.inventory_status EQ '1'>Active Equipment</cfif>	
               ...
               <cfif session.inventory_status EQ '3'>Equipment In Repair</cfif>) </span>  <img src="images/clear.gif" width="275" height="1" /> <a href="inventory_add.cfm" class="admin-records">Add New Inventory</a> </div>
...
<tr> 
   ...
   <th align="center">Purch<br />Date</th>
...
WHERE  [Employee ID] =  #COMPUTED_COLUMN_14# 
   ...
   <td align="center"><a href="inventory_repair_damage_history_list.cfm?ID=#ID#">History</a></td>
        </tr>
  </cfoutput>
