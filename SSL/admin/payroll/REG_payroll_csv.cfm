<cflock scope = "Application"     timeout = "30" type = "Exclusive">
  <cfquery name="delete_csv" datasource="jrgm" >
	DELETE FROM  csv_payroll_file 
  </cfquery>
 ...
 FROM app_pay_periods WHERE pay_period_number = #pay_period_number#
  ...
  <cfquery name="get_employees_with_time" datasource="jrgm"     >
		...
		WHERE  app_employee_payroll_clock.Time_In > #pay_period_start# AND  app_employee_payroll_clock.Time_In < #end_date_plus1#  AND FWW <> 1  AND APP_employees.branch ='#url.branch#'
 ...
 WHERE Employee_ID IN (#mylist#) AND app_employee_payroll_clock.Time_In > #pay_period_start# AND  app_employee_payroll_clock.Time_In < #end_date_plus1#
 AND in_out_status =2
  </cfquery>
      ...
      <cfloop query="get_employees_with_time" >
        <tr>
          ...
          <cfoutput>
            ...
            <cfif  get_employees_with_time.Employee_ID EQ  10561>
              <td>1056</td>
              <cfelseif  get_employees_with_time.Employee_ID EQ  15971>
              <td>1597</td>
              <cfelseif  get_employees_with_time.Employee_ID EQ  29661>
              <td>2966</td>
              <cfelseif  get_employees_with_time.Employee_ID EQ  15761>
              <td>1576</td>
              <cfelseif  get_employees_with_time.Employee_ID EQ  27621>
              <td>2762</td>
              <cfelseif  get_employees_with_time.Employee_ID EQ  26871>
              <td>2687</td>
              <cfelseif  get_employees_with_time.Employee_ID EQ  15581>
              <td>1558</td>
              <cfelseif  get_employees_with_time.Employee_ID EQ  10581>
              <td>1058</td>
              <cfelseif  get_employees_with_time.Employee_ID EQ  11261>
              <td>1126</td>
       ...
       <!--- <CFSET  file_number = #LEFT(get_employees_with_time.Employee_ID,4)#>--->
       
       <cfif Len(get_employees_with_time.Employee_ID) EQ 6>
       <CFSET  file_number = #RIGHT(get_employees_with_time.Employee_ID,4)#>
       <cfelse>
       <CFSET  file_number = #LEFT(get_employees_with_time.Employee_ID,4)#>
       </cfif>
       
        <cfquery name="insert_csv" datasource="jrgm" >
	INSERT INTO csv_payroll_file ( company_code,batch_id,first_name,last_name,file_number,reg_hours,ot_hours )
      VALUES       ( 'AZW',#branchcode#,'#get_employees_with_time.first_name#','#get_employees_with_time.last_name#',#file_number#,#hourssum_REG_TOTALS#.#NumberFormat(base60reg,"09")#,#hourssum_OT_TOTALS#.#NumberFormat(base60ot,"09")#)
  </cfquery>
      </cfloop>
    </tbody>
  </table>
  ...
  <br />
  <br />
  <cfif BRANCH EQ 'Newport News' >
    <cfset BRANCH ='NewportNews'>
  </cfif>
  <cfquery name="get_csv" datasource="jrgm" >
SELECT * FROM csv_payroll_file 
  </cfquery>
  ...
  <cfheader name="Content-Disposition" value="attachment; filename=#BRANCH#.csv">
  
  <!--- Output Column Headers ---> 
  <cfoutput>Co Code #chr(delim)# Batch ID #chr(delim)# First Name #chr(delim)# Last Name #chr(delim)# File Number #chr(delim)# REG Hours #chr(delim)# OT Hours #chr(delim)#</cfoutput> <cfoutput>#chr(13)#</cfoutput> 
  <!--- line break after column header ---> 
  
  <!--- Spill out data from a query --->
  <cfloop query="get_csv">
    <cfoutput>#ucase(company_code)# #chr(delim)# #ucase(batch_id)# #chr(delim)# #ucase(first_name)# #chr(delim)# #ucase(last_name)# #chr(delim)# #ucase(file_number)# #chr(delim)# #NumberFormat(reg_hours,"999.99")# #chr(delim)# #NumberFormat(ot_hours,"999.99")# </cfoutput> <cfoutput>#chr(13)#</cfoutput>
  </cfloop>
  </body>
  </html>
</cflock>