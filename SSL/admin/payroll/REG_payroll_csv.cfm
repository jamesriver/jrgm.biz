...
<cfoutput>Co Code #chr(delim)# Batch ID #chr(delim)#  First Name #chr(delim)#  Last Name #chr(delim)#  File Number #chr(delim)#  REG Hours #chr(delim)# OT Hours #chr(delim)#</cfoutput>
<cfoutput>#chr(13)#</cfoutput> 
<!--- line break after column header --->

<!--- Spill out data from a query --->
<cfloop query="get_csv">
    <cfoutput>#ucase(company_code)# #chr(delim)# #ucase(batch_id)#   #chr(delim)# #ucase(first_name)#  #chr(delim)# #ucase(last_name)#
	#chr(delim)# #ucase(file_number)#  #chr(delim)# #NumberFormat(reg_hours,"999.99")#  #chr(delim)#  #NumberFormat(ot_hours,"999.99")#
	</cfoutput>
    <cfoutput>#chr(13)#</cfoutput>
</cfloop> 
  </body>
