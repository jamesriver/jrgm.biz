    ...
    <div class="centrecontent_inner">
      <table width="100%"  border="0" cellpadding="2" cellspacing="0">
        <cfoutput query="app_payroll_periods">
          <tr class="job_data">
            <td><strong>Unapproved Payroll for <cfoutput>#url.branch#</cfoutput> <br />
              for the period #DateFormat("#begin_date#", "mm/dd/yyyy")#- #DateFormat("#pay_period_end#", "mm/dd/yyyy")#</strong><img src="images/clear.gif" width="250" height="1" /><a href="payrollBetty.cfm">Back to Payroll Main Page</a></td>
          </tr>
        </cfoutput>
      </table>
      <br />
      <cfquery name="get_employees_with_time" datasource="jrgm" >
		...
		WHERE branch = '#url.branch#'  AND app_employee_payroll_clock.Time_In > '#DateFormat(ds_date, "yyyy-mm-dd")# 00:00:00.000' AND  app_employee_payroll_clock.Time_In < #end_date_plus1#
      ...
      <table   border="0" cellspacing="0" cellpadding="2" class="sortable" >
		...
		ON app_employee_payroll_clock.Employee_ID=APP_employees.[Employee ID]  
		WHERE  
         (app_employee_payroll_clock.Time_In > '#DateFormat(ds_date, "yyyy-mm-dd")# 00:00:00.000' AND app_employee_payroll_clock.Time_Out < #end_date_plus1#)  AND Employee_ID IN (#mylist#)  AND payroll_approved IS NULL
		ORDER by APP_employees.last_name
 </cfquery>
