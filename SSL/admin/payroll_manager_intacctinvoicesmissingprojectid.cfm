...
<cfhttp url="http://api.jrgm.com/external_api/intacct.php?auth=jrgmAPI!&type=invoicelineitems_missing_projectid" method="get" result="httpResp" timeout="30">
...
<cfif IsDefined('intacct_response.data')>
    <cfloop from="1" to="#arrayLen(intacct_response.data)#" index="i">
      <cfset ArrayAppend(lineitems, [intacct_response.data[i].intacct_entrydate, intacct_response.data[i].intacct_recordid, intacct_response.data[i].intacct_lineno, intacct_response.data[i].intacct_itemname])>
    </cfloop>
</cfif>
