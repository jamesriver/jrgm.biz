...
<!---    <cfquery name="getpix" datasource="jrgm">
     SELECT ID,image1 AS myimage,LEN(image1) AS myimage2 FROM 
       app_notes  
       ORDER by ID desc
          </cfquery>
         ...
         <!---  
          <cfquery 
name="GetBLOBs" datasource="jrgm">
SELECT ID,image1
FROM app_notes  where id  =155
</cfquery>
<cfset i = 0>
<table border=1>
    <cfoutput query="GetBLOBS">
        <tr>
            ...
            <td>
            <cfset i = i+1>
            <cfset myImage=ImageNew("#GetBLOBS.image1#")>
...
<!---  
 <cfquery name="selectQuery" datasource="jrgm">
  select    image1
  from      app_notes  where id  =155
</cfquery>
 --->
