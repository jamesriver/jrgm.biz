    ...
    <cfloop collection=#form# item="form_element_name">
      ...
      <cfif field_name neq "fieldnames" AND field_name neq 'submittedform'>
        ...
        <cfif field_name EQ 'Job_ID'>
            <cfset query_quote_start_parameters = query_quote_start_parameters & field_name & "='" & field_value & "', ">
        <cfelse>
...
<cfquery name="get_all_jobs" datasource="jrgm"  >
    SELECT aj.[Job ID] as Job_ID, aj.[Wk Location Name] as Project_Name FROM app_jobs aj
    WHERE aj.Status='IN PROGRESS'
      AND aj.branch='#get_quote_start.q_branch#'
    ORDER BY aj.[Wk Location Name]
</cfquery>
<cfset active_jobs = ArrayNew(1)>
<cfloop query="get_all_jobs">
    <cfset ArrayAppend(active_jobs, [Job_ID, Project_Name])>
</cfloop>
          ...
          <table class="table">
            ...
            <tr>
              ...
              <td colspan="4">Associated Project:
                <select name="Job_ID">
                    <option value="">[ Create a new one when quote is approved ]</option>
                    <cfloop from="1" to="#arrayLen(active_jobs)#" index="i">
                        <cfoutput>
                            <option value="#active_jobs[i][1]#">#active_jobs[i][2]#</option>
                        </cfoutput>
                    </cfloop>
                </select>
              </td>
