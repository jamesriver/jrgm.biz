...
<body onunload="closechildwindow()"><div id="outerfull">
...
<cfif get_branch_employees_missing.recordcount EQ 0>
 <cfif IsDefined("form.EMPLOYEEID") ><span class="redarial">This employee has been added to this job</span><br /></cfif>
  ...
  In order to add employees to this job they must be on the Daily sheet. </span><cfoutput>
        <p>
		<form>  <button type="button" onclick="javascript:window.close()">Close</button></form>
		 </p>
      </cfoutput> 
<cfabort>
</cfif>
     
     <form   method="post" action="daily_sheet_edit_add_employee_job.cfm"  >
