...
<cfloop index="i" list="#form.fieldnames#">
 ...
 Select ID FROM app_App_Employee_Payroll_Clock where id=IDENT_CURRENT('App_Employee_Payroll_Clock') 
 </cfquery>
    <cfif Left(DB_field,8) is "TIMEOUTA">
      ...
      <CFSET newtime = #parsedatetime(dte&tim)#>
      <cfquery name="jrgm_time" datasource="jrgm">  
    UPDATE App_App_Employee_Payroll_Clock 
   ...
   WHERE ID = #get_max.ID#
    ...
    SELECT DATEDIFF(mi,time_in,time_out) AS 'Duration'  FROM app_App_Employee_Payroll_Clock WHERE ID =#get_max.ID#
    </cfquery>
      <cfquery name="update_time" datasource="jrgm">
    UPDATE App_App_Employee_Payroll_Clock SET time_worked = #calculate_time.Duration#  WHERE ID =#get_max.ID#
    </cfquery>
    </cfif>
    <!------------------------------------------------------------------>
