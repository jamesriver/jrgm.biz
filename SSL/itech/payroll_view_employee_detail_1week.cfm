...
<cfset tomorrow = dateadd("d",1,somedate)>
 ...
 <cfquery name="get_employees_with_time" datasource="jrgm"  >
		...
		WHERE supervisor = #SESSION.userid#  AND (app_employee_payroll_clock.Time_In > #pay_period_start# AND app_employee_payroll_clock.Time_Out < #pay_period_end_week_plusone#)
...
<cfparam name="employee_total" default="0">
 ...
 UPDATE App_Employee_Payroll_Clock SET payroll_approved = NULL, approved_by = NULL WHERE   Employee_ID IN (#employeelist#)  AND (app_employee_payroll_clock.Time_In > #pay_period_start# AND app_employee_payroll_clock.Time_Out < #pay_period_end_week_plusone#)
 </cfquery>
 <CFSET mylist = #Form.EMPLOYEE_ID#>
<cfloop  list = "#mylist#"   index = "i" >
 
  <cfquery name="update_time" datasource="jrgm">
 UPDATE App_Employee_Payroll_Clock SET payroll_approved = 1, approved_by = #SESSION.userid#  WHERE   Employee_ID = #i# AND (app_employee_payroll_clock.Time_In > #pay_period_start# AND app_employee_payroll_clock.Time_Out < #pay_period_end_week_plusone#)
 </cfquery></cfloop>
 <CFSET flag = 'uncheck'>
...
SELECT  Employee_ID , SUM(time_worked) As sumdailytime, in_out_status,ds_date 
 ...
 GROUP by Employee_ID, ds_date, in_out_status
       ...
       FROM app_employee_payroll_clock
       WHERE "Employee_ID" =#get_employees_with_time.Employee_ID# AND (app_employee_payroll_clock.Time_In > #pay_period_start# AND app_employee_payroll_clock.Time_Out < #pay_period_end_week_plusone#)
         </cfquery>
