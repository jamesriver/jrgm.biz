...
SELECT     ID, date_created, gross_margin, opportunity_id, date_updated, user_id,  total_contract_price, contract_adjustment, discount_adjustment, contract_installments
FROM         quote_main 
where opportunity_id  =#url.id#
</cfquery>
<cfset total_adjusted_contract_price = get_quote_dollars.total_contract_price + get_quote_dollars.contract_adjustment + get_quote_dollars.discount_adjustment>
<cfif  get_quote_dollars.total_contract_price NEQ ""  AND  get_quote_dollars.total_contract_price NEQ "0.00"  AND   get_quote_dollars.contract_installments NEQ "0.00">
  <CFSET monthly_price =  total_adjusted_contract_price/get_quote_info_date.contract_installments>
  <cfelse>
  <CFSET monthly_price = "">
</cfif>
<!---BA-  <cfoutput> #get_quote_info.Billing_Address_2#  </cfoutput>--->
...
SELECT       ID, [Job ID] AS ProjectID, [Wk Loc ID], [Wk Location Name] AS projectname, [Service Address] as project_work_location_address, [Service Address2]   AS project_work_location_address2, [Service City] as project_work_location_city, [Service State] as project_work_location_state, [Service Zip] as project_work_location_zip, project_type,
                       active_record, branch, Responsible_User_Employee_ID, 
                      ...
                      Crew_Leader_ID, billto_company_name,contract_installments, last_quote_id, project_status_checkbox, [Wk Loc ID] as insightly_id
