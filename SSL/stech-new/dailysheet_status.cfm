...
<cfquery name="get_daily_sheets_for_supervisor" datasource="jrgm"  >
        ...
        <cfif IsDefined("form.submitted")  AND form.dsid NEQ "" >AND    ID = #form.dsid#</cfif>   AND ds_approved IS NULL AND ds_date > '05/31/2014' 
...
FROM get_all_employee_info WHERE employee_ID =#get_daily_sheets_for_supervisor.supervisor_id#
        </cfquery>
                    <cfquery name="get_info_CL" dbtype="query" cachedWithin="#createTimeSpan( 0, 1, 0, 0 )#">
SELECT   last_name AS employee_name,branch
FROM   get_all_employee_info WHERE employee_ID =#get_daily_sheets_for_supervisor.Crew_Leader_ID#
    </cfquery>
                    <tr>
