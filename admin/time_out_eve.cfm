...
<cfif CGI.HTTP_USER_AGENT EQ 'CFSCHEDULE' >
 ...
 WHERE  [Employee ID] =#get_open_workers.crew_leader#
        ...
        <cfmail to="benchanviolin@gmail.com"    FROM="benchanviolin@gmail.com"   subject="JRGM Employee Auto Time Out" type="html">
          ...
          <br />
          #get_CL_name.first_name# #get_CL_name.last_name#'s crew was auto timed out at 5:00 PM today.<br />
          The daily sheet ID is #get_open_workers.ds_id#<br />
        </cfmail>
      </cfloop>
      <!---  <cfabort> --->
      <!----------------------- This BELOW was here prior to 4/29/14 ------------------------------->
      <cfquery name="get_open_workers" datasource="jrgm" >
...
#get_super_name.firstname# #get_super_name.lastname# - #get_super_name.email#<br /><br /> --->
